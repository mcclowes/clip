...
The first release bundles the catalog for offline use. Upgrade CLIP to get catalog updates, then run `clip refresh` to inspect the rendered skill diff and any mutation-marker changes. `clip refresh --accept <tool>` adopts a reviewed update; `--accept-all` is for CI. You can always register a newer local schema with `--schema`.
