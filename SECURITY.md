...
- **Authorization.** Schemas describe capabilities; they never grant permissions. A wrong `mutating: false` marker in the bundled registry is a safety defect: report it as an issue, or privately if it hides a destructive command. [docs/spec.md](docs/spec.md#threat-model) describes the registry as a supply chain into agent context.
