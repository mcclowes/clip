...
function parseCli() {
  const { positionals, values: options } = parseArgs({ allowPositionals: true, options: {
    ...
    file: { type: 'string' }, trust: { type: 'string', multiple: true }, accept: { type: 'string', multiple: true }, 'accept-all': { type: 'boolean' }, all: { type: 'boolean' }, write: { type: 'boolean' }, strict: { type: 'boolean' }, scope: { type: 'string' }, version: { type: 'boolean' },
