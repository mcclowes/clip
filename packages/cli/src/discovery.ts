...
const probeMaxBytes = 1024 * 1024;

const pathDirectories = () => (process.env.PATH ?? '').split(delimiter).filter(Boolean);
/** A command containing a slash is a path; anything else is looked up on PATH. */
const candidatePaths = (command: string) => (command.includes('/') ? [resolve(command)] : pathDirectories().map(dir => resolve(dir, command)));
...
export function probeSchema(executable: string, command: string) {
  ...
  const result = spawnSync(executable, [command], { encoding: 'utf8', timeout: probeTimeoutMs, killSignal: 'SIGKILL', maxBuffer: probeMaxBytes, stdio: ['ignore', 'pipe', 'pipe'] });
